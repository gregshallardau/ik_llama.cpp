...
| `-rtrd, --run-time-repack-dual` | Keep both the original and the run-time-repacked layout of every eligible tensor resident, and pick per phase | - | Prompt processing keeps the original layout (where `iqk_mul_mat` converts weight tiles on the fly and wins at large batch), token generation uses the repacked one - so you get the prompt-processing speed of no `-rtr` together with the generation speed of `-rtr`, instead of having to pick one. Costs roughly 2x the bytes of the repack-eligible tensors, which is most of a transformer's weights; use `--repack-dual-filter` to narrow that. Unlike `-rtr` it does not disable mmap. |
| `--repack-dual-max-tokens N` | Batch size at or below which `-rtrd` uses the repacked layout | 32 | 32 is where `iqk_mul_mat`'s on-the-fly conversion starts to beat the statically repacked kernels for most types. |
| `--repack-dual-filter REGEX` | Only give a second copy to tensors whose name matches REGEX | all eligible | Use this when the full 2x does not fit, e.g. `--repack-dual-filter 'attn'` to dual-path the attention weights only and leave the (much larger) MoE experts single-copy. |
